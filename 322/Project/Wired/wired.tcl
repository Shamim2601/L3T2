...
# Files
# ==============================================================================
set trace_file_name			$num_node\_$num_flow\_$cbr_pckt_rate.tr
set nam_file_name			$num_node\_$num_flow\_$cbr_pckt_rate.nam
# set topo_file_name			$num_node\_$num_flow\_$cbr_pckt_rate.topo
set directory				"Output/"
# set queue_trace_file_name   $num_node\_$num_flow\_$cbr_pckt_rate.out
...
# set queue_trace_file [open $directory$queue_trace_file_name w]
...
for {set i 0} {$i < $num_flow} {incr i} {
	set tcp_($i) [new $source_type]
	set sink_($i) [new $sink_type]
	$tcp_($i) set class_ $i
	$tcp_($i) set fid_ $i
	# $udp_($i) set windowOption_ 15
	$tcp_($i) set packetSize_ 960
...
for {set i 0} {$i < $num_random_flow} {incr i} {
	...
	$ns_ attach-agent $node_($source_number) $tcp_($k)
  	$ns_ attach-agent $node_($sink_number) $sink_($k)
...
proc finish {} {
    puts "finishing\n"
    global ns_ tracefd namtrace nam_file_name
    $ns_ flush-trace
    close $tracefd
	# close $topo_file
    close $namtrace
